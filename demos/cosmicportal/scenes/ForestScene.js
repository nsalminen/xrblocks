...
export const ForestScene = {
  ...
  body: /* glsl */ `
    ...
    col = mix(skyLow, skyMid, smoothstep(-0.3, 0.4, p.y));
    col = mix(col, skyTop, smoothstep(0.4, 1.0, p.y));

    // Crescent moon glow up high.
    {
      vec2 mp = vec2(0.55, 0.65);
      float dr = length(p - mp);
      ...
      float bite = smoothstep(0.06, 0.05, length(p - (mp + vec2(0.02, 0.01))));
      col += vec3(1.0, 0.97, 0.90) * max(disc - bite, 0.0) * 1.5;
    }

    // Stars in upper sky.
    {
      float s1 = starsLayer(vUv * 2.0, 60.0, 0.985);
      float s2 = starsLayer(vUv * 2.0 + 5.0, 130.0, 0.992);
      float upper = smoothstep(-0.1, 0.5, p.y);
    ...
    // ---- Distant tree silhouettes (back row, hazy) ----
    {
      for (int i = 0; i < 9; i++) {
        ...
        float baseY = -0.4;
        float t = treeMask(p, cx, w, h, baseY);
    ...
    // Forest floor.
    if (p.y < -0.55) {
      float floorN = fbm(vec2(p.x * 4.0, p.y * 4.0));
      vec3 floorCol = mix(vec3(0.04, 0.06, 0.04),
                          vec3(0.10, 0.12, 0.06), floorN);
      col = mix(col, floorCol, smoothstep(-0.55, -1.0, p.y));
    }

    // Ground mist rolling in (always on, animated).
    {
      float mist = fbm(vec2(p.x * 3.0 + uTime * 0.1, p.y * 6.0));
      mist *= smoothstep(-0.2, -0.7, p.y);
    ...
    // ---- Distant lightning storm flashes every 8s ----
    {
      ...
      float bolt = smoothstep(0.005, 0.0,
          abs(p.x - (bx + sin(p.y * 18.0 + k) * 0.04)))
